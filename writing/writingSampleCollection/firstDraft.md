...
This literature review examines the state-of-the-art in Deep Learning Recommendation Models (DLRMs), focusing on their architectural design, industrial deployment strategies, and parameter synchronization mechanisms. The scope encompasses three primary research themes: the fundamental architecture and training paradigms of DLRMs, production system designs that decouple training and inference clusters, and parameter update strategies that balance model freshness, accuracy, and system overhead. This review synthesizes findings from both academic research and industry-scale deployments. By critically analyzing existing approaches and identifying their limitations, this review establishes the foundation for understanding the tradeoffs inherent in maintaining fresh, accurate recommendation models at petabyte scale.
...
This literature review has synthesized the current state of DLRM architectures, industrial deployment strategies, and parameter synchronization mechanisms. While significant progress has been made in scaling DLRMs to petabyte scale, fundamental limitations persist in achieving near-real-time parameter updates without compromising accuracy or system stability. Existing approaches are constrained by bandwidth bottlenecks inherent in decoupled architectures. The identification of CPU underutilization at inference nodes and the intrinsic low-rank structure of embedding updates suggests a paradigm shift toward co-locating lightweight training within inference clusters. However, realizing this potential requires addressing critical challenges: dynamic rank adaptation and performance isolation. This review establishes the foundation for understanding tradeoffs and opportunities in DLRM parameter synchronization, setting the stage for research that can simultaneously achieve near-real-time freshness, maintain model accuracy, and minimize system overhead.
